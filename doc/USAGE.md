# armstrong_pick_ik : Usage

## Using armstrong_pick_ik as a Kinematics Plugin

As discussed in the MoveIt 2 documentation, you can use the [MoveIt Setup Assistant](https://moveit.picknik.ai/humble/doc/examples/setup_assistant/setup_assistant_tutorial.html) or change the [`kinematics.yaml` file](https://moveit.picknik.ai/main/doc/examples/kinematics_configuration/kinematics_configuration_tutorial.html?highlight=kinematics%20yaml#kinematics-configuration) for your robot setup to use `armstrong_pick_ik` as the IK solver.

An example `kinematics.yaml` file might look as follows:

```yaml
panda_arm:
  kinematics_solver: armstrong_pick_ik/PickIkPlugin
...
For an exhaustive list of parameters, refer to the [parameters YAML file](../src/armstrong_pick_ik_parameters.yaml).
...
Additionally, you might want to define values for the `cost_threshold`, `approximate_solution_cost_threshold`, and `stop_optimization_on_valid_solution` parameters to decide when armstrong_pick_ik will stop optimizing for your cost function and which solutions it should accept.

Alternatively, consider adding your own cost functions to the `armstrong_pick_ik` source code (specifically, in [`goal.hpp`](../include/goal.hpp) and [`goal.cpp`](../src/goal.cpp)) and submit a pull request with the new functionality you add.
