...
TEST_CASE("armstrong_pick_ik::make_frame_tests") {
    ...
    SECTION("One goal, one test") {
        auto const test_fns =
            armstrong_pick_ik::make_frame_tests({zero_frame}, position_epsilon, orientation_epsilon);
        CHECK(test_fns.size() == 1);
    }

    SECTION("Zero threshold") {
        auto const test_fns = armstrong_pick_ik::make_frame_tests({zero_frame}, 0.0, 0.0);
    ...
    SECTION("Goal is frame, but orientation is different") {
        ...
        auto const test_fns_pos_only = armstrong_pick_ik::make_frame_tests({zero_frame}, position_epsilon);
        CHECK(test_fns_pos_only.at(0)({zero_frame_rotated}) == true);
    }
}

TEST_CASE("armstrong_pick_ik::make_pose_cost_fn") {
    auto const zero_frame = Eigen::Isometry3d::Identity();
    ...
    SECTION("Goal is frame") {
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 0.0, 0.0);
        CHECK(cost_fn({zero_frame}) == Catch::Approx(0.0));
    }

    SECTION("Goal is frame, with position scale") {
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 1.0, 0.0);
        CHECK(cost_fn({zero_frame}) == Catch::Approx(0.0));
    }

    SECTION("Goal is frame, with rotation scale") {
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 1.0, 0.5);
        CHECK(cost_fn({zero_frame}) == Catch::Approx(0.0));
    }

    SECTION("Goal is second index") {
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(translate_y2_frame, 1, 1.0, 0.0);
    ...
    SECTION("Translation in two axes, square of distance") {
        ...
        auto const const_fn = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 1.0, 0.5);
    ...
    SECTION("Zero position scale with translation") {
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 0.0, 0.5);
    ...
    SECTION("Negative position scale same as zero position scale") {
        auto const cost_fn_zero = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 0.0, 0.5);
        auto const cost_fn_neg = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, -1.0, 0.5);
    ...
    SECTION("Negative rotation scale same as zero rotation scale") {
        auto const cost_fn_zero = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 1.0, 0.0);
        auto const cost_fn_neg = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 1.0, -0.5);
    ...
    SECTION("Rotation in one axis") {
        ...
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, 1.0, 1.0);
    ...
    SECTION("Rotation in one axis, scaled 0.5") {
        ...
        auto const cost_fn =
            armstrong_pick_ik::make_pose_cost_fn(zero_frame, 0, position_scale, rotation_scale);
    ...
    SECTION("Test 0 - sample data taken from instrumenting bio_ik") {
        ...
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(goal, 0, position_scale, rotation_scale);
...
TEST_CASE("armstrong_pick_ik::make_pose_cost_functions") {
    Eigen::Isometry3d const goal =
        Eigen::Translation3d(0.3327501714229584, -0.025710120797157288, 0.5902695655822754) *
    ...
    SECTION("Function is same as armstrong_pick_ik::make_pose_cost_fn") {
        auto const cost_fn = armstrong_pick_ik::make_pose_cost_fn(goal, 0, position_scale, rotation_scale);
        auto const cost_fns =
            armstrong_pick_ik::make_pose_cost_functions({goal}, position_scale, rotation_scale);
    ...
    SECTION("Two goals, two functions") {
        auto const cost_fns =
            armstrong_pick_ik::make_pose_cost_functions({goal, frame}, position_scale, rotation_scale);
