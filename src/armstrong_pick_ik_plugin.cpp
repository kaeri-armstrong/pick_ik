...
bool PickIKPlugin::searchPositionIK(std::vector<geometry_msgs::msg::Pose> const& ik_poses,
    ...
    if (cost_function) {
        for (auto const& pose : ik_poses) {
            goals.push_back(
                Goal{make_ik_cost_fn(pose, cost_function, robot_model_, jmg_, ik_seed_state), 1.0});
        }
    }
...
std::vector<std::string> const& PickIKPlugin::getJointNames() const { return joint_names_; }

std::vector<std::string> const& PickIKPlugin::getLinkNames() const { return link_names_; }

bool PickIKPlugin::getPositionFK(std::vector<std::string> const&,
                                 std::vector<double> const&,
                                 std::vector<geometry_msgs::msg::Pose>&) const {
    return false;
}

bool PickIKPlugin::getPositionIK(geometry_msgs::msg::Pose const&,
                                 std::vector<double> const&,
                                 std::vector<double>&,
...
bool PickIKPlugin::searchPositionIK(geometry_msgs::msg::Pose const& ik_pose,
                                    std::vector<double> const& ik_seed_state,
                                    double timeout,
                                    std::vector<double>& solution,
                                    moveit_msgs::msg::MoveItErrorCodes& error_code,
                                    kinematics::KinematicsQueryOptions const& options) const {
